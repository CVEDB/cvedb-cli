...
type TaskGroupStats struct {
	...
	Median                  cvedb.Duration `json:"median,omitempty"`
	MedianAbsoluteDeviation cvedb.Duration `json:"median_absolute_deviation,omitempty"`
...
func CalculateTaskGroupStats(sj cvedb.SubJob) TaskGroupStats {
	stats := TaskGroupStats{
		Count: len(sj.Children),
		MinDuration: TaskDuration{
			TaskIndex: -1,
			Duration:  cvedb.Duration{Duration: time.Duration(math.MaxInt64)},
		},
		MaxDuration: TaskDuration{
			TaskIndex: -1,
			Duration:  cvedb.Duration{Duration: time.Duration(math.MinInt64)},
		},
	}
	...
	for _, child := range sj.Children {
		...
		if child.FinishedDate.IsZero() {
			taskDuration.Duration = cvedb.Duration{Duration: time.Since(child.StartedDate)}
		} else {
			taskDuration.Duration = cvedb.Duration{Duration: child.FinishedDate.Sub(child.StartedDate)}
		}
	...
	if len(taskDurations) >= 2 {
		...
		var absoluteDeviations []cvedb.Duration
		for _, d := range taskDurations {
			diff := d.Duration.Duration - stats.Median.Duration
			if diff < 0 {
				diff = -diff
			}
			absoluteDeviations = append(absoluteDeviations, cvedb.Duration{Duration: diff})
		}
