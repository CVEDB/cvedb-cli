...
func SubJobTimeRange(subJob cvedb.SubJob, rangeEnd time.Time) TimeRange {
	end := rangeEnd
...
func run(cfg *Config) error {
	...
	var filteredSubJobs []cvedb.SubJob
	...
	filteredSubJobs = cvedb.LabelSubJobs(filteredSubJobs, *workflowVersion)
...
func constructRunURL(workflowID uuid.UUID, runID uuid.UUID) string {
	return fmt.Sprintf("https://cvedb.io/editor/%s?run=%s", workflowID, runID)
}
