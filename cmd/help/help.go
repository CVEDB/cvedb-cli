...
func generateHelpMarkdown(workflow *cvedb.Workflow, labeledPrimitiveNodes []*cvedb.PrimitiveNode, labeledNodes []*cvedb.Node, workflowSpec config.WorkflowRunSpec, runs []cvedb.Run, maxMachines int) string {
	...
	sb.WriteString(fmt.Sprintf("- [View on Cvedb](%s)\n", workflowURL))
	sb.WriteString("- [Learn more about executing workflows](https://github.com/cvedb/cvedb-cli#execute)")
...
func constructWorkflowURL(workflow *cvedb.Workflow) string {
	return fmt.Sprintf("https://cvedb.io/editor/%s", workflow.ID)
}
